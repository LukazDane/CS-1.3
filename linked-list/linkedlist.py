...
class LinkedList(object):
    ...
    def get_at_index(self, index):
        ...
        if index == 0:
            ...
            while temp is not None and i < index+1:
                # print("\tlooking for index:{} we are at:{} thus we see:{}".format(index, i, temp))
                if i == index:
                    return temp.data
                else:
                    temp = temp.next
    ...
    def delete(self, item):
        ...
        Best case running time: O(1) item is head or tail and easy to find
        Worst case running time: O(n) item is somewhere else or not in the linked list"""
        # Start at the head node
        node = self.head
